...
// ----------------------------------------------------------------------------------------------------------------- //
	...
	var vVideoID = window.innerHeight > window.innerWidth ? vVideoID_Celular : vVideoID_Computador ;

// ----------------------------------------------------------------------------------------------------------------- //

	// var vJsonSetup = 'https://vdlvry.com/setups/SOV/lead2.json' ;
	
	// var vCssBootstrap = 'https://cdn.jsdelivr.net/npm/bootstrap@5.2.2/dist/css/bootstrap.min.css' ;
	
	// var vCssPlayer = 'https://vdlvry.com/videoboxes/universal_player/player.css' ;
	
	// var vCssVideo  = 'https://vjs.zencdn.net/7.11.4/video-js.css' ;
	
	// var vJsVideo   = 'https://vdlvry.com/videoboxes/videojs/video.js' ;
	
	// var vJsStream  = 'https://vdlvry.com/videoboxes/videojs/videojs-http-streaming.js' ;
...
// ----------------------------------------------------------------------------------------------------------------- //
					...
					.appendChild(
						obj.htmlToElement(
							'<div class="player-wrapper">' +
								'<iframe  id="' +
								obj.config.host +
								'_stream" src="https://player.vimeo.com/video/' +
								$config.video_id +
								"?controls=" +
								(obj.config.controlls ? 1 : 0) +
								"&autoplay=" +
								(obj.config.autoplay ? 1 : 0) +
								'&muted=1" frameborder="0"  allowfullscreen  allow=autoplay ></iframe>' +
								'<div class="glass" onclick="UPplayer.togglePause()"></div>' +
								'<div class="play" onclick="UPplayer.togglePlay()" style="z-index:3"></div>' +
								'<div class="playpause" onclick="UPplayer.togglePlay()" style="z-index:3"></div>' +
								'<a class="soundButton" id="unmute" onclick="UPplayer.toggleMute()"></a>' +
					...
					.appendChild(
						obj.htmlToElement(
							'<div class="player-wrapper">' +
								...
								"&autoplay=" +
								(obj.config.autoplay ? 1 : 0) +
								'&muted=1&texttrack=en" frameborder="0"  allowfullscreen  allow=autoplay ></iframe>' +
								'<div class="glass" onclick="UPplayer.togglePause()"></div>' +
								'<div class="play" onclick="UPplayer.togglePlay()" style="z-index:3"></div>' +
								'<div class="playpause" onclick="UPplayer.togglePlay()" style="z-index:3"></div>' +
								'<a class="soundButton" id="unmute" onclick="UPplayer.toggleMute()"></a>' +
		...
		obj.cloudflare = function ($config) {
			document
				...
				.appendChild(
					obj.htmlToElement(
						'<div class="player-wrapper"><iframe src ="https://iframe.videodelivery.net/' +
							$config.video_id +
							'" id="' +
							obj.config.host +
							'_stream" allow="accelerometer; gyroscope; autoplay; encrypted-media; picture-in-picture;"></iframe>' +
							'<div class="glass" onclick="UPplayer.togglePause()"></div>' +
							'<div class="play" onclick="UPplayer.togglePlay()" style="z-index:3"></div>' +
							'<div class="playpause" onclick="UPplayer.togglePlay()" style="z-index:3"></div>' +
							'<a class="soundButton" id="unmute" onclick="UPplayer.toggleMute()"></a>' +
			...
			obj.addScript("https://embed.videodelivery.net/embed/sdk.latest.js", function () {
				...
				player.muted = true;
				if (obj.config.autoplay) player.play();
...
// ----------------------------------------------------------------------------------------------------------------- //
    ...
    // obj.addLink( vCssVideo );

    // --------------------------------------------------------------------------- //

    obj.addScript( vJsVideo , function (){

    // --------------------------------------------------------------------------- //

    obj.addScript( vJsStream , function (){
...
// ----------------------------------------------------------------------------------------------------------------- //
						...
						.appendChild(
							obj.htmlToElement(
								'<div class="player-wrapper">' +
									'<video id="' +
									obj.config.host +
									'_stream"  class="video-js vjs-default-skin" playsinline ' +
									(obj.config.autoplay ? "autoplay" : "") +
									" muted >" +
									'<source src="' +
									$config.video_id +
					...
					var player = videojs(obj.config.host + "_stream", { autoplay: obj.config.autoplay ? "muted" : "", controls: false });
					player.playbackRate(obj.config.playback_rate);
					if (!obj.config.autoplay) {
						document.querySelector(".up_container .soundButton").style.display = "none";
						document.querySelector(".up_container .play").style.visibility = "visible";
		...
		obj.vidalytics = function ($config) {
			...
			document
				...
				.appendChild(
					obj.htmlToElement(
						'<div class="player-wrapper"><div ><div id=' +
							video_id +
